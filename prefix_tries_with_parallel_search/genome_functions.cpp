...
char *genomeFunctions::getrandomfragment(int size)
{
    srand(time(NULL));
    long long index = rand() % (genome_size - size);
    cout << "Chosen index: " << index << endl;
    char *fragment = new char[size + 1];
    memcpy(fragment, genome_data + index, size);
    fragment[size] = '\0';
    return fragment;
}
